...
-/
import Connes.Foundation.LinearAlgebra.PaperFiniteCharts
...
def chartQuadraticData (χ : C →ₗ[k] k) (N : ℕ) (s : Fin 3) :
    PaperFiniteCharts.QuadraticData (PaperFiniteCharts.CoeffIndex N) := by
  classical
...
theorem chartEvaluation_isQuadratic (χ : C →ₗ[k] k) (N : ℕ) (s : Fin 3) :
    PaperFiniteCharts.IsQuadratic (chartEvaluation χ N s) := by
...
theorem quadratic_support_card_bound
    ...
    (hdeg : PaperFiniteCharts.IsQuadratic P) (hP : P ≠ 0) :
    Fintype.card (ι → F) ≤ 4 * PaperFiniteCharts.weightOn P := by
  have hweight := PaperFiniteCharts.quadratic_weight_lower_bound P hdeg hP
...
theorem chart_support_card_bound (χ : C →ₗ[k] k) (N : ℕ)
  ...
  let S := PaperFiniteCharts.supportOn (chartEvaluation χ N s)
  have hsubset :
