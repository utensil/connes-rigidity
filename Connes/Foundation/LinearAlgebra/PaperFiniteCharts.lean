...
def extendCoefficients {N M : ℕ} (hNM : N ≤ M) (v : Fin N → F) : Fin M → F :=
...
/- Quadratic coefficient data on an arbitrary finite Boolean coordinate set.
Paper: Lemma 4.3. -/
...
def IsQuadratic {ι : Type*} [Fintype ι]
    (P : (ι → F) → F) : Prop :=
  ∃ q : QuadraticData ι, ∀ x, q.eval x = P x

def supportOn {ι : Type*} [Fintype ι] (P : (ι → F) → F) : Finset (ι → F) :=
  by
    classical
    exact Finset.univ.filter (fun x => P x ≠ 0)

def weightOn {ι : Type*} [Fintype ι] (P : (ι → F) → F) : ℕ :=
  (supportOn P).card

/- The Boolean degree-two weight estimate in chart coordinates. Paper: Lemma 4.3. -/
theorem quadratic_weight_lower_bound
    {ι : Type*} [Fintype ι] (P : (ι → F) → F)
    (hdeg : IsQuadratic P) (hP : P ≠ 0) :
    weightOn P ≥ 2 ^ (Fintype.card ι - 2) := by
  classical
  obtain ⟨q, hq⟩ := hdeg
  have hchar2F (a : F) : a + a = 0 := by
    rw [← two_smul F a, BooleanPolynomial.two_eq_zero, zero_smul]
  ...
  have hcard := BooleanPolynomial.quadratic_support_quarter
    q.eval b hadd hbadd hf
  have hweight :
