...
/- Distinct finite dual coordinates give distinct evaluation functionals. Paper: §4. -/
lemma evalStar_sum_ne_zero {v w : SymplecticIndex} (hvw : v ≠ w) :
    PaperDualTopology.evalStar v + PaperDualTopology.evalStar w ≠ 0 := by
  intro hzero
  have h := congrArg (fun f : VStar => f (Pi.single v 1)) hzero
  simpa [PaperDualTopology.evalStar, Pi.single_apply, hvw] using h
...
theorem aCoordinateEmbedding_injective :
  ...
  change (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
    ...
    (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
      PaperDualTopology.evalStar w) at hfirst
  by_contra hvw
  have hzero :
      PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
        (PaperDualTopology.evalStar v + PaperDualTopology.evalStar w) = 0 := by
    calc
      PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
          (PaperDualTopology.evalStar v + PaperDualTopology.evalStar w) =
        (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
            PaperDualTopology.evalStar v) +
          (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
            PaperDualTopology.evalStar w) := by
              rw [TensorProduct.tmul_add]
      _ = (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
            PaperDualTopology.evalStar w) +
          (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
            PaperDualTopology.evalStar w) := by rw [hfirst]
      _ = 0 := by
        calc
          (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
              PaperDualTopology.evalStar w) +
            ...
            (1 : k) • (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
              PaperDualTopology.evalStar w) +
              (1 : k) • (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
                PaperDualTopology.evalStar w) := by simp
          _ = ((1 : k) + 1) •
              (PaperFiniteCharts.basisVector 0 ⊗ₜ[k]
                PaperDualTopology.evalStar w) := by
            rw [add_smul]
  ...
  have hcontract := congrArg
    (fun z : PaperKernel.AVStar =>
      aZeroCoeff (PaperICC.paper_contractStar
        (PaperICC.paper_evalStar (Pi.single v 1)) z)) hzero
  rw [PaperICC.paper_contractStar_tmul, map_zero, map_smul,
    aZeroCoeff_basisVector_zero] at hcontract
  have hone : (1 : k) = 0 := by
    simpa [smul_eq_mul, PaperICC.paper_evalStar,
      PaperDualTopology.evalStar, Pi.single_apply, hvw] using hcontract
