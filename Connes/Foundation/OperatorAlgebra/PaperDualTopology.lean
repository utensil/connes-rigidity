...
/- Evaluation on a tensor is the corresponding finite coordinate of the
dual map.  This is the bridge from the compact character topology to the
raw `A → V` coordinates. Paper: §3. -/
def evalStar (v : OpenAIPort.SymplecticIndex) : PaperKernel.VStar where
  toFun f := f v
  map_add' f g := by simp
  map_smul' a f := by simp

theorem avDualEquiv_apply_evalStar (F : Module.Dual k PaperKernel.AVStar)
    (a : Construction.A) (v : OpenAIPort.SymplecticIndex) :
    (PaperDualCoordinates.avDualEquiv F a) v =
      F (a ⊗ₜ[k] evalStar v) := by
  have h := congrArg (fun g : PaperKernel.VStar →ₗ[k]
      Module.Dual k Construction.A => g (evalStar v) a)
