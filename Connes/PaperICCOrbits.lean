...
lemma transvection_action_apply_source {i j : Fin 3} (hij : i ≠ j)
    (a : SpecialLinear.R) (x : Construction.A) :
    sl3AAction (Matrix.SpecialLinearGroup.transvection hij a) x j = x j := by
  change (Matrix.mulVec (Matrix.transvection i j a) x) j = x j
  rw [Matrix.transvection, Matrix.add_mulVec, Matrix.one_mulVec,
    Matrix.single_mulVec_eq]
  simp [hij]
...
lemma paper_avStar_coeff_sl3 (s : SpecialLinear.SL3) (u : PaperKernel.AVStar)
  ...
  · simp
  · intro a f
    simp [avStarAction, tensorProductLinearEquiv, paper_vStarBasis]
  · intro x y hx hy
...
/-- The second action reduces to the first on the SL₃ factor. Paper: §5. -/
lemma paper_thetaTwo_sl3 (s : SpecialLinear.SL3) (d : PaperKernel.D) :
    thetaTwoLinearMap (s, (1 : PaperKernel.Q)) d =
      (avStarAction s (1 : PaperKernel.Q) d.1, sl3CAction s d.2) := by
  rcases d with ⟨u, c⟩
  apply Prod.ext
  · change avStarAction s (1 : PaperKernel.Q) u +
        thetaTwoTermMap (s, (1 : PaperKernel.Q)) c =
      avStarAction s (1 : PaperKernel.Q) u
...
def paper_evalStar (v : PaperKernel.PaperV) : PaperKernel.VStar →ₗ[k] k where
  toFun f := f v
  map_add' f g := by simp
  map_smul' a f := by simp

def paper_contractStar (ψ : PaperKernel.VStar →ₗ[k] k) :
    PaperKernel.AVStar →ₗ[k] Construction.A :=
...
@[simp] lemma paper_contractStar_tmul
    (ψ : PaperKernel.VStar →ₗ[k] k) (a : Construction.A)
    (f : PaperKernel.VStar) :
    paper_contractStar ψ (a ⊗ₜ[k] f) = ψ f • a := by
  rfl

def paper_a0 : Construction.A := fun _ => 1

lemma paper_a0_ne_zero : paper_a0 ≠ 0 := by
  intro h
  have h0 := congrFun h (0 : Fin 3)
  simp [paper_a0] at h0

lemma paper_tmul_star_ne_zero (f : PaperKernel.VStar)
    (hf : f ≠ 0) : (paper_a0 ⊗ₜ[k] f : PaperKernel.AVStar) ≠ 0 := by
  ...
  intro hzero
  have hcontract := congrArg (paper_contractStar (paper_evalStar v)) hzero
  rw [map_zero, paper_contractStar_tmul] at hcontract
  change f v • paper_a0 = 0 at hcontract
  have hmul : f v * (1 : k) = 0 := by
    simpa [paper_a0, smul_eq_mul] using congrFun hcontract (0 : Fin 3)
...
lemma paper_q_coord_difference_ne_zero (q : PaperKernel.Q) (hq : q ≠ 1) :
  ...
  have hf : f ≠ 0 := by
    ...
    have h := congrArg (fun z : PaperKernel.VStar =>
      z + paper_coordStar p) hzero
    dsimp [f] at h
    rw [add_assoc, Connes.Construction.SymmetricTensor.add_self_eq_zero, add_zero,
      zero_add] at h
    exact h
  exact ⟨f, hf, p, rfl⟩

lemma paper_avStar_q_action (q : PaperKernel.Q) (f : PaperKernel.VStar) :
    avStarAction (1 : SpecialLinear.SL3) q (paper_a0 ⊗ₜ[k] f) =
      paper_a0 ⊗ₜ[k] qVStarActionHom q f := by
...
theorem paper_q_displacement_one (q : PaperKernel.Q) (hq : q ≠ 1) :
  ...
  have hf : f ≠ 0 := by
    ...
    simp [f, paper_coordStar, Pi.single_apply] at h
  have hdiff : qVStarActionHom q f + f ≠ 0 := by
    intro hzero
    apply hp
    have h := congrArg (fun z : PaperKernel.VStar => z + f) hzero
    rw [add_assoc, Connes.Construction.SymmetricTensor.add_self_eq_zero, add_zero,
      zero_add] at h
    simpa [f] using h
  let d : PaperKernel.D := (paper_a0 ⊗ₜ[k] f, 0)
  refine ⟨d, ?_⟩
  intro hzero
  have hfirst := congrArg Prod.fst hzero
  have hstar : paper_a0 ⊗ₜ[k]
      (qVStarActionHom q f + f) = 0 := by
    rw [TensorProduct.tmul_add]
    simpa [d, paper_thetaOne_q_zero, paper_avStar_q_action, add_comm] using hfirst
...
theorem paper_q_displacement_two (q : PaperKernel.Q) (hq : q ≠ 1) :
  ...
  have hstar : paper_a0 ⊗ₜ[k]
      (qVStarActionHom q f + f) = 0 := by
    rw [TensorProduct.tmul_add]
    simpa [d, paper_thetaTwo_q_zero, paper_avStar_q_action, add_comm] using hfirst
  exact paper_tmul_star_ne_zero _ hdiff hstar
...
def paper_actionData_one : PaperICC.ActionData paperThetaOneHom where
...
def paper_actionData_two : PaperICC.ActionData paperThetaTwoHom where
...
def paper_dataPair : PaperICC.DataPair PaperKernel.paperActionData where
