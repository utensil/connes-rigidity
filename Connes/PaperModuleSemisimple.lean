...
Concrete semisimplicity of the first quotient module in Zhou §4 and §6.
...
/- The symplectic pairing identifies the finite dual with the defining module. Paper: §4. -/
...
/- The symplectic pairing has trivial kernel. Paper: §4. -/
...
/- Equal finite dimensions turn the pairing injection into an equivalence. Paper: §4. -/
...
/- The pairing intertwines the quotient action. Paper: §4. -/
...
/- The finite quotient acts transitively on nonzero vectors. Paper: §4. -/
theorem qV_transitive_on_nonzero_test :
    ∀ v : PaperV, v ≠ 0 → ∀ w : PaperV, w ≠ 0 → ∃ q : Q,
      q • v = w := by
  exact Sp4.transitive_on_nonzero_vectors

/- The finite dual quotient action is transitive on nonzero functionals. Paper: §4. -/
...
/- The finite dual action is packaged as a group representation. Paper: §4. -/
def qVStarRepresentation : Representation k Q W where
  ...
  map_one' := by
    ...
    simpa using congrArg (fun e : W ≃ₗ[k] W => e f)
...
/- The finite dual representation is nontrivial. Paper: §4. -/
...
/- Transitivity makes the finite dual representation simple. Paper: §4. -/
...
/- The quotient action on `AVStar` is recorded as a representation. Paper: §4. -/
def avStarActionQEquivHom : Q →* (AV ≃ₗ[k] AV) :=
  PaperKernel.avStarActionHom.comp qToH

/- The tensor summand carries the actual quotient action. Paper: §4. -/
...
/- The direct sum of copies of the finite dual representation is explicit. Paper: §4. -/
def directSumRepresentation : Representation k Q DS :=
  Representation.directSum (fun _ : I => qVStarRepresentation)

/- The direct sum is identified with its finitely supported coordinate model. Paper: §4. -/
...
/- The ordered tensor basis identifies `AVStar` with the direct sum coordinates. Paper: §4. -/
...
/- The tensor coordinate equivalence respects the quotient representations. Paper: §4. -/
...
/- The tensor equivalence becomes a group-algebra linear map. Paper: §4. -/
...
/- The direct sum action and its pointwise finitely supported action agree. Paper: §4. -/
...
/- The tensor summand is semisimple over the finite group algebra. Paper: §4. -/
...
/- The one-dimensional trivial quotient representation is simple. Paper: §4. -/
def scalarRepresentation : Representation k Q k :=
  Representation.trivial k Q k

/- The trivial scalar module is simple over the group algebra. Paper: §4. -/
...
/- The coordinate type-synonym modules are linearly equivalent over the group algebra. Paper: §4. -/
def cPointwiseEquiv :
...
/- The trivial coordinate representation is semisimple. Paper: §4. -/
...
/- The `C` summand is semisimple by transport through its chosen basis. Paper: §4. -/
...
/- The actual trivial `C` representation is semisimple. Paper: §4. -/
...
/- The first quotient representation is the product of the tensor and `C` actions. Paper: §4. -/
def firstProductRepresentation : Representation k Q (AV × PaperKernel.C) :=
  avStarRepresentation.prod cRepresentation

/- The tensor summand embeds as a group-algebra submodule of the product. Paper: §4. -/
...
/- The `C` summand embeds as a group-algebra submodule of the product. Paper: §4. -/
...
/- The concrete first quotient module is semisimple. Paper: §4. -/
