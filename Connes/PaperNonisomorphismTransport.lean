...
import Connes.PaperQuotientModuleTransport
...
theorem paperCharacteristicModuleEquiv
    ...
    ∃ σ : PaperKernel.Q ≃* PaperKernel.Q, Nonempty
      ((qRepresentationOne PaperKernel.paperActionData).asModule ≃ₗ[Ring]
        (qRepresentationTwoAlong PaperKernel.paperActionData σ).asModule) := by
...
theorem paperNotIsomorphic
    (hOne : moduleOneSemisimple PaperKernel.paperActionData) :
    ¬ GroupsIsomorphic
  ...
  exact Semisimple.semisimple_invariant_under_linear_equiv e hOne
