...
Paper-facing ICC transfer for Zhou §5. The generic semidirect ICC lemma in
`Connes/ICC.lean` incorrectly assumes that the acting group is ICC; Zhou's
acting group is `SL₃(R) × Sp₄(F₂)`, so this file proves the required three-case
criterion directly on the actual product quotient.
-/
...
private theorem q_displacement_conjugate
    (action : H →* MulAut N) (x : Carrier action)
    (hs : x.right.1 = 1) (q : Q) (hqx : x.right.2 = q) (hq : q ≠ 1)
    (a : N) (ha : a * (action ((1 : S), q) a)⁻¹ ≠ 1) :
    ∀ s : S,
      (⟨x.left * action (s, (1 : Q))
  ...
  · change
      (⟨x.left * action (s, (1 : Q))
          (a * (action ((1 : S), q) a)⁻¹), x.right⟩ : Carrier action).left =
        ((SemidirectProduct.inl (action (s, (1 : Q)) a) : Carrier action) * x *
          (SemidirectProduct.inl (action (s, (1 : Q)) a) : Carrier action)⁻¹).left
    simp only [SemidirectProduct.left_inl, SemidirectProduct.mul_left,
      SemidirectProduct.inv_left, SemidirectProduct.right_inl,
      SemidirectProduct.mul_right, SemidirectProduct.inv_right]
    simp only [one_mul, mul_one, inv_one, map_one, MulAut.one_apply,
      map_mul, map_inv]
    have hdisp' :
        action (s, (1 : Q)) a *
            action (s, (1 : Q)) ((action ((1 : S), q) a)⁻¹) =
  ...
  · change
      (⟨x.left * action (s, (1 : Q))
          (a * (action ((1 : S), q) a)⁻¹), x.right⟩ : Carrier action).right =
        ((SemidirectProduct.inl (action (s, (1 : Q)) a) : Carrier action) * x *
          (SemidirectProduct.inl (action (s, (1 : Q)) a) : Carrier action)⁻¹).right
...
theorem isICC_of_product_quotient
  ...
  · rintro (x : Carrier action) hx
    ...
    · let conjugates : S → Carrier action := fun s =>
      ...
      simp only [conjugates, projectionS, SemidirectProduct.right_inr,
        SemidirectProduct.mul_right, SemidirectProduct.inv_right]
      rfl
              ...
              (⟨x.left * action (s, (1 : Q)) displacement, x.right⟩ :
                ...
                (SemidirectProduct.inl (action (s, (1 : Q)) a) :
                  ...
                  (SemidirectProduct.inl (action (s, (1 : Q)) a) :
                    Carrier action)⁻¹
            dsimp [displacement]
            exact q_displacement_conjugate action x hs' x.right.2 rfl hq a ha s⟩
